...
## Implementation Status (DONE 2026-04-20)

| Task                                                         | Status  |
| ------------------------------------------------------------ | ------- |
| Task 1: Install auth dependencies                            | ✅ Done |
| Task 2: Extend Prisma schema + migration                     | ✅ Done |
| Task 3: Add SMTP env vars to config schema                   | ✅ Done |
| Task 4: Add shared auth types + Zod schemas                  | ✅ Done |
| Task 5: Create ArgonService                                  | ✅ Done |
| Task 6: Create MailService                                   | ✅ Done |
| Task 7: Create auth DTOs                                     | ✅ Done |
| Task 8: AuthService — register, login, session CRUD          | ✅ Done |
| Task 9: AuthService — refresh token rotation + logout        | ✅ Done |
| Task 10: AuthService — password change + reset               | ✅ Done |
| Task 11: AuthService — account deletion                      | ✅ Done |
| Task 12: Passport strategies, guards, @CurrentUser decorator | ✅ Done |
| Task 13: Create AuthController                               | ✅ Done |
| Task 14: Create UsersModule + UsersController                | ✅ Done |
| Task 15: Wire AuthModule + cookie-parser + throttling        | ✅ Done |
| Task 16: e2e test — register → login → refresh → logout      | ✅ Done |
...
Note: Zod strips unknown keys silently, so a test that only calls `safeParse` with extra keys would pass even before the schema change. The failing test must check for coercion behavior (number conversion) that only works once the field is declared in the schema:
...
Expected: FAIL — `result.data.SMTP_PORT` is `undefined` (not `587`) because the field isn't in the schema yet.
